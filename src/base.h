...
#define SPI_RESET_YES   1
#define SPI_RESET_NO    0

#define MAX_SCAN_STEP   256 * 1024
...
typedef struct _calibration
{
	...
	char data;//保存半径，深度，厚度的值
	char data_measure[20];//保存检测到的实际值
	double distance;
	int    CalibrationWedgeDelay ;
	//ultrasound->tcg
	int   bNewPoint ;
	unsigned int   BeamDelay [setup_MAX_GROUP_LAW_QTY];
...
typedef struct AUTO_PROBE__
{
	char strCompanyName[8];
	char nType ;
	char strProbeName[20] ;
	char nProbeQty     ;
	char nFreq[2]        ;
	char nElemPitch[2]   ;
	char id[4]           ;
...
typedef struct tmp_config 
{
	...
	guchar  WedgeSeparation_reg;


	LAW_BEAM	focal_law_all_beam[setup_MAX_GROUP_QTY][setup_MAX_GROUP_LAW_QTY];
	LAW_ELEM	focal_law_all_elem[setup_MAX_GROUP_QTY][setup_MAX_GROUP_LAW_QTY][setup_MAX_ELEM_RX_ACTIVE];

	DOT_TYPE   scan_data[setup_MAX_GROUP_QTY][640*400];
	int		   measure_data[setup_MAX_LAW_QTY][8];//接收从硬件发过来的数据
	double     field[2][4];//保存4个field最后用于显示的值
	...
	double  current_angle[setup_MAX_GROUP_QTY]; // current angle
	...
	unsigned char	scan_type[16];	/* 16个窗口显示的类型 */
	char	scan_group[16];	/* 16个窗口对应的group */
	int	    scan_xpos[16];
	int	    scan_ypos[16];
	int     scan_width[16]  ;
	int     scan_height[16] ;

	unsigned char	_scan_type[16];	/* 16个窗口显示的类型 */
	char	_scan_group[16];	/* 16个窗口对应的group */
	int	    _scan_xpos[16];
	int	    _scan_ypos[16];
	int     _scan_width[16]  ;
	int     _scan_height[16] ;

	unsigned char gateShow[4];

	int     clb_wedge_max_DB[setup_MAX_GROUP_LAW_QTY] ;
	int     clb_wedge_position[setup_MAX_GROUP_LAW_QTY];
	int     clb_sensation_max_DB[setup_MAX_GROUP_LAW_QTY] ;
	int     clb_tcg_position[setup_MAX_GROUP_LAW_QTY];
	int     clb_tcg_max_DB[setup_MAX_GROUP_LAW_QTY];
	...
	tcg_data_spi    tcg_spi[setup_MAX_GROUP_QTY]  ;
...
{
	int    DataSize ;              // data size of each save unit
	int    MaxStoreIndex ;         // how many unit of DMA data 256M space can store
	int    StepsPerResolution ;    // encoder Steps/resolution

	int    DataSaveIndex      ;    // current beam save in 256M space index
	int    EncoderPosition    ;    // current encoder counter

	//char   ScanRefreshRequest ;    // whether restart DMA
	//char   ScanDmaCircleTimes ;    // DMA circle times in the 256M memory

	int    EncoderType   ;         // timer:0  encoder1:1 encoder2:2
	//int    RangeStart    ;         // scan start position
	//int    RangeStop     ;         // scan stop  position
	//int    RangeStep     ;         // scan resolution  pulse/resolution
	int    ScanStepQty       ;         // beam quantity in this scan
	// B scan display information
	int    BscanShowStart     ;         // data start index in display
	int    BscanShowStop      ;         // data stop  index in display
	int    BscanShowBeamQty   ;         // quantity of beams displayed in screen
	// C scan display information
	int    CScanShowStart     ;         // data start index in display
	int    CScanShowStop      ;         // data stop  index in display
	int    CScanShowBeamQty   ;         // quantity of c data displayed in screen
	//
	int    ScanZeroOffset     ;         // origin position offset in save data index
    //long int StartTimeSec     ;
    //long int StartTimeUSec    ;
}SCAN_INFO ,*SCAN_INFO_P ;
...
typedef struct Draw_interface 
{
	...
	GtkWidget       *softkey_button[58];
	Display			*disp;
	FakeKey			*fk;
	GtkWidget		*sub_window;	/*弹出窗口*/
	...
	//gint    clb_count;
	double  vel;
	guint    gate_a_start[ setup_MAX_GROUP_QTY ][ setup_MAX_GROUP_LAW_QTY ];
	guint    gate_a_end  [ setup_MAX_GROUP_QTY ][ setup_MAX_GROUP_LAW_QTY ];
	guint    gate_b_start[ setup_MAX_GROUP_QTY ][ setup_MAX_GROUP_LAW_QTY ];
	guint    gate_b_end  [ setup_MAX_GROUP_QTY ][ setup_MAX_GROUP_LAW_QTY ];
	guint    gate_i_start[ setup_MAX_GROUP_QTY ][ setup_MAX_GROUP_LAW_QTY ];
	guint    gate_i_end  [ setup_MAX_GROUP_QTY ][ setup_MAX_GROUP_LAW_QTY ];
    int      bGateSynchro ;

	int      thickness_refract_times ;
	gboolean  key_flag;	/*caps lock 标志位*/
	GtkTextBuffer *TextBuffer_all;
	guint	keyboard_xpos;
	guint	keyboard_ypos;

	SCAN_INFO  ScanInfor ;
	char       ScanStart ; // button 732:  scan start
	unsigned char nEncoderType ;
	unsigned int  nPointQty[setup_MAX_GROUP_QTY] ;
	int     bSaveDataProcessing;
...
#define GROUP_VAL_POS(a, b)	(pp->p_config->group[a].b)				/* a表示哪个group b是返回哪个值 */

#define LAW_VAL_POS(a, b)  (pp->p_config->group[a].law_info.b)

#define GROUP_GATE_POS(a)   GROUP_VAL_POS(pp->p_config->groupId,gate[GROUP_VAL_POS(pp->p_config->groupId , gate_pos)].a)
#define GROUP_GATE_VAL_POS(a , b , c)  GROUP_VAL_POS(a , gate[b].c )
#define GROUP_CURSORS_POS(a) GROUP_VAL_POS(pp->p_config->groupId , cursors_info[GROUP_VAL_POS(pp->p_config->groupId ,selection)-5].a)
