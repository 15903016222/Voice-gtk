...
static const gchar *content_en25[] = {
	/* 二级菜单第5页 */
	"Select", "Position", "Characterize", "Parts", "Advanced"
};

static const gchar *content_en26[] = {
	/* 二级菜单第6页 */
	"Configuration", "Beam Angle", "Focal Point", "Aperture", NULL//"Laws"
};

static const gchar *content_en27[] = {
	/* 二级菜单第7页 */
	"Encoder", "Inspection", "Area", "Start", NULL //"Data"
};
...
static const gchar *content_en29[] = {
	/* 二级菜单第9页 */
	"Pref.", "System", "Service", "Network", "Update"
};
...
static const gchar *content_en200[] = {
	...
	"Tx/Rx mode","Pulser","Receiver","Connection R","Geometry",
	"Thickness","Diameter","Material"
};
...
static const gchar *content_en202[] = {
	...
	"Depth 2", "Thickness 1", "Thickness 2",	"Next", "Gate Width", "Gate Threshold","Get Position","Radius A",
	...
	"Get Position", "Get Position2" , "VPA" , "LAST VPA" , "Wedge Delay" , "Wedge Sep." , "PCS" ,
	"Thickness", "Ref Point" , "Next Section" ,"Position" , "Gain" , "Next Point" , "Add Point" , "Cancel" ,
	"Accept TCG" , "U(r)" ,"U(m)", "Gate Start"
};

static const gchar *content_en203[] = {
	/* 二级菜单第0页下的第3页三级菜单 */
	"Back", "Start", "Weld", "Symmetry", NULL, NULL,
	"Next","Weld land height", "Weld land offset","Finish","Fill zone height", "Fill zone angle",
	"Fill zone radius",  "Weld Show","Root Height","Root Angle","Fill Angle","Root Angle2","Fill zone Angle","Weld Root Angle","Fill zone angle1"
};
...
static const gchar *content_en210[] = {
	/* 二级菜单第1页下的第0页三级菜单 */
	"Gain", "Start", "Range", "Wedge Delay" , "Velocity",  "WaveType", "Gain-r", "Gain(Ref.)"
};
...
static const gchar *content_en220[] = {
	/* 二级菜单第2页下的第0页三级菜单 */
	"Gate", "Parameters", "Gate Start", "Gate Width", "Gate Threshold", "Switch", "Synchro.", "Measure", "RF"
};
...
static const gchar *content_en231[] = {
	/* 二级菜单第3页下的第1页三级菜单 */
	"  Selection  ","  %(r)  ", "  %(m)  ","  U(r)  ", "  U(m)  ", NULL ,
	"S(r)", "S(m)", "Data Link", "I(r)", "I(m)", "S(r&m)",  "Angle",
	"Amplitude",  "UT",  "Scan",  "Index", "Add Entry" , "VPA"
};
...
static const gchar *content_en243[] = {
	/* 二级菜单第4页下的第4页三级菜单 */
	"  Scan  ","  Color  ", "  Envelope  ", "  Appearance  ", "  Overlay  ", "  Source  ","Compression", "Optimum","Ratio 1:1","Interpolation","Color","Orientation"
};
...
static const gchar *content_en251[] = {
	/* 二级菜单第5页下的第1页三级菜单 */
	"  Scan Offset  ","  Index Offset  ", "  Skew(°) ", "PCS" ,NULL, NULL
};
...
static const gchar *content_en253[] = {
	/* 二级菜单第5页下的第3页三级菜单 */
	"  Geometry  ","  Thickness  ", "  Diameter  ","  Material  ", "Load",  NULL
};

static const gchar *content_en254[] = {
	/* 二级菜单第5页下的第4页三级菜单 */
	"Load Part", "Clear Part", "Scale", NULL, NULL, NULL
};
...
static const gchar *content_en262[] = {
	/* 二级菜单第6页下的第2页三级菜单 */
	"Type.", "Focal Depth", "Position End",  "Position Step", "Depth End", NULL, "Offset Start", "Offset End","Depth Start", "Focal Radius"
};
...
static const gchar *content_en271[] = {
	/* 二级菜单第7页下的第1页三级菜单 */
	"  Type  ","  Scan  ", "  Index ","  Scan Speed  ", "Scan Speed", "Index Speed","  Scan Speed  ", "Index Speed",
};

static const gchar *content_en272[] = {
	/* 二级菜单第7页下的第2页三级菜单 */
	"  Scan Start  ","  Scan End  ", " Resolution ","  Index Start  ", "  Index End  ", " Resolution "
};
...
const gchar *units_en[] = 
{
	...
	"(s)", "(°)", "(Step/mm)", "(rpm)", "(dB/in)","(dB/μs)","(in/s)","VPA"," " ,"MHz"
};
...
const gchar *all_menu_content_en[] = 
{
...
#if FPGA_VERSION > 1
    "100 Low", "200 Middle", "400 High",
    "100", "200", "400",
#else
    "50 Low", "100 Middle", "200 High",
    "50", "100", "200",
#endif
	...
	"A-scan", "B-scan", "C-scan", "S-scan", "R and M", "Reference", "Measure", "Data",  "Strip Chart",/* 263 Selection */
	...
	"A-B-S  A-Scan B-Scan S-Scan", "A-C-[C] A-Scan C-Scan [C-Scan]", "A-S-[C]  A-Scan S-Scan [C-Scan]", "[PA]-TOFD  [PA]-TOFD [A-S] A-B", "[PA]-TOFD  [PA]-TOFD [A-C] A-B",
	"Strip Chart-[A]  Strip Chart[A-Scan]",								 /* 285 Display */
	"A", "B", "C", "S", "A-B", "A-B-C", "A-B-S", "A-C-[C]", "A-S-[C]", "PA TOFD-[S]", "PA TOFD-[C]", "Strip Chart-[A]", /* 296 Displ   */
	...
	"A-Scan", "B-Scan", "C-Scan",  "FFT", "S-Scan", "Strip Chart",					/* 348 Scan */
...
/*	"AL OXIDE", "ALUMINUM", "BERYLIUM", "BRASS", "CADMIUM", "COPPER", "STEEL.MILD",	*/		/* 399 Material */
	...
	"Quad", "Up", "Down", "Clock/Dir", "Clock/Dir Up", "Clock/Dir Down", "Quad Up", "Quad Down", "Clicker",/* 424 Type */
	...
	"Storage Card   \"\\Storage Card\"", "Internal Memory  \"\\User\"",	"USB Storage   \"\\Hard Disk\"",			/* 453 Storage */
	"Storage Card", "Internal Memory","USB Storage",								/* 455 Storage */
...
const gchar *field1_en[]	= 
{
	...
	"AWS-D A  Indication level for AWS-D1.5 A",
	"AWS-D B  Zero reference level for AWS-D1.5 B",
	"AWS-D C  Attenuation factor for AWS-D1.5 C",
	"AWS-D D  Indication rating for AWS-D1.5 D",
	"AWS-D 45  Discontinuity Severity Class for AWS-D1.5 45",
	"AWS-D 60  Discontinuity Severity Class for AWS-D1.5 60",
	"AWS-D 70  Discontinuity Severity Class for AWS-D1.5 70",
	"AWS-D CL  Discontinuity Severity Class for AWS-D1.5",
	"E%  Peak amplitude of the envelope in gate A",
	"E^  Position of envelop in gate A",
	"E-3dB  Envelop signal at -3 dB in gate A",
	"E-6dB  Envelop signal at -6 dB in gate A",
	"E-12dB  Envelop signal at -12 dB in gate A",
	"E-20dB  Envelop signal at -20 dB in gate A",
	"API-DL  Defect length extension according to API code",
	"API-HW  Half-wave width according to API code",
	"Scale  Scale factor",
	"%(U(r))  Signal amplitude at reference ultrasonic cursor position", 
	"%(U(m))  Signal amplitude at measurement ultrasonic cursor position"		  /* 203 Field */
};
const gchar *field2_en[]	= 
{
	"A%  Peak amplitude in gate A",
	"AdBA  Signal in gate minus gate level for gate A",
	"AdBr  Diff. between signal and reference in gate A",
...
const gchar *field_en[]		=
{
	...
	"AWS-D A", "AWS-D B", "AWS-D C", "AWS-D D", "AWS-D 45",
	"AWS-D 60", "AWS-D 70", "AWS-D CL",
	//"E%", "E/", "E-3dB", "E-6dB", "E-12dB", "E-20dB", "API-DL",
	//"API-HW",
	//"Scale", "%(U(r))", "%(U(m))",
	"DA-DI",
	"T-D(r)" , "T-D(m)" ,"T-S(m-r)" ,"T-D(m-r)"
};
const gchar *field_en_[]		=
{
	"A%", "AdBA", "AdBr", "B%", "BdBB", "BdBr", "A/", "B/", "I/", "I(w)/", 
	"T(A/)", "ML", "%(r)", "%(m)", "%(m-r)", 
	"U(r)", "U(m)", "U(m-r)", "P(r)", "P(m)", "P(m-r)", "S(r)", "S(m)", 
...
const gchar *field_unit_en_mm[]		=
{
	...
	"mm", "%",  "%",  "%", "%",   "mm", "mm", "mm", "mm", "mm", 
	"mm", "mm", "mm", "mm", "mm", "mm", "mm", "mm", "mm", "mm", 
	"mm", "mm", "mm", "mm", "mm", "mm", "mm", "mm", "mm", "times",
	"times", "dB","dB", "dB", "dB", "  ", "  ", "  ", "  ", "%",
	"%",  "%",  "mm", "mm", "mm", "mm", "mm", "X",  "%",  "%"
	, "mm", "mm", "mm"
};
