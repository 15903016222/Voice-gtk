...
#ifndef LANGUAGEDICT_H_
#define LANGUAGEDICT_H_

#include <stddef.h>
#include "language.h"
typedef const char* languageStruct[LANGUAGE_MAX];
#if 1//说明：用到就在后面打   //#
static languageStruct stringDict[] =
{
...
//	/*_STRING_Software_Update,*/{"SW Update","软件升级"},	//#
	/*_STRING_Calibrate_Touch_Screen,*/{"Calibrate TS","屏幕校准"},	//#
	...
	/*_STRING_Manual_Weld,*/{"LIST","列表"},
	/*_STRING_Auto_Weld,*/{"LIST","列表"},
	...
	/*_STRING_GROUP_ADD_NOT_AVAILABEL,*/{"Resource limited, Group add invalidate!" ,"系统资源受限,组添加失败！"} ,
	...
	/*_STRING_TOFD_Depth,*/{"T-D(r) TOFD Ultrasonic Axis Reference Cursor Depth" ,"T-D(r) TOFD超声轴方向参考光标深度"},
	/*_STRING_TOFD_Depth,*/{"T-D(m) TOFD Ultrasonic Axis Measure Cursor Depth" ,"T-D(m) TOFD超声轴方向测量光标深度"},
	/*_STRING_TOFD_Length,*/{"T-S(m-r) TOFD Scan Axis Cursors Space" ,"T-S(m-r) TOFD扫查轴参考光标与测量光标间距"},
	/*_STRING_TOFD_Height,*/{"T-D(m-r) TOFD Ultrasonic Axis Cursors Height" ,"T-D(m-r) TOFD超声轴方向参考光标与测量光标高度"},
	...
	/*_STRING_Select_Average,*/{"Please Select Average AScan with S(m)" ,"请用S(m)测量线选择目标波形"},
	...
	/*_STRING_DISK_VOLUME_LITTLE,*/{"No enough space, Save Failed!" ,"空间不足，保存失败！"},
	/*_STRING_DISK_VOLUME_THRESHOLD,*/{"File Save Finished, No enough space, Delete some data." ,"文件保存成功,剩余空间不足,请清理数据!"},
