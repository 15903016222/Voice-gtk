...
typedef struct Beam_angle
{
    ...
    int      beam_type; //0:扇形聚焦  1:线性聚焦
    int      beam_angle_sel;    //0:通过折射角来计算入射角,再计算声程; 1:主轴偏置; 2:声束与主轴投影面存在夹角.
...
typedef struct FocusParameter
{
    ...
    float field_distance[MAXNUM];//每一束的中心正元到出射点间的距离
    
}PARAMETER, *PARAMETER_P;


extern void focal_law(PARAMETER_P data);/*回调函数*/
