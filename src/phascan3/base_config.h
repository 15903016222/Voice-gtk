...
typedef struct _sizing_curve
{
	...
	int             dac_ref_ampl[setup_MAX_GROUP_LAW_QTY] ;
	unsigned int    amplitude[setup_MAX_GROUP_LAW_QTY][16];
	int             position[setup_MAX_GROUP_LAW_QTY][16];
	unsigned int    delay; //data_232  linear dac , start point
} SIZING_CURVES, *SIZING_CURVES_P;
...
typedef struct _AWS_D_15
{
	int    bCalibrated    ;
	int    nPosition      ;
	double nReferenceGain ;
}AWS_D_15 , *AWS_D_15_P;
...
struct _Group 
{
	...
	/* 发射接收 */
	unsigned char	pulser1;		/* 1~128 - elem_qty(聚焦阵元数最大为32) + 1 指定发射阵元与机器配置相关我们是128阵元最大,值与connect P 一样 */
	...
	short			per_reference;	/* 参考光标的高度 */
	short			per_measure;	/* 测量光标的高度 */
	int				u_reference;	/* 参考光标在超声轴的位置 */
	int				u_measure;		/* 测量光标在超声轴的位置 */
	...
	unsigned short	gain_offset[setup_MAX_GROUP_LAW_QTY];
    unsigned int    beam_delay[setup_MAX_GROUP_LAW_QTY];
	float           field_distance[setup_MAX_GROUP_LAW_QTY];
	st_PART	        part;
...
struct _Config 
{
	...
	unsigned char 	fieldIndex[2][4];	//add by lzk
	...
	unsigned int    weld_display_on_off;
	...
	*/
	...
	unsigned int    damping_pos_ut1;
	unsigned int    damping_pos_ut2;
	//int    G_delay[1024];//保存每一个beam的延时
	...
	*/
	st_Enc	encoder1[2];
	char    szPalettePathAmplitude[52];
...
extern unsigned int	get_damping_pos(CONFIG *p);
extern void		set_damping_pos(CONFIG *p, unsigned int data);
extern unsigned int get_damping_pos(CONFIG *p);
/* fetch material info of current config 获取当前配置中材料的信息 */
extern char* get_material_name (int nPos_);
extern unsigned int	get_material_lw (int pos);
extern unsigned int	get_material_sw (int pos);

/* part operations 工件的操作 */
extern unsigned char	get_part_geometry (int grp);
extern void		set_part_geometry (int grp, void* data);
extern unsigned char	get_part_material (int grp);
extern void		set_part_material (int grp, void* data);
extern unsigned int	get_part_thickness (int grp);
extern void		set_part_thickness (int grp, void* data);
extern unsigned int	get_part_diameter (int grp);
extern void		set_part_diameter (int grp, void* data);
extern unsigned char get_part_weld (int grp);
extern void set_part_weld (int grp, void* data);
extern unsigned char get_part_symmetry (int grp);
extern void set_part_symmetry (int grp, void* data);
extern unsigned int	get_weland_height (int grp);
extern void		set_weland_height (int grp, void* data);
extern unsigned int	get_weland_offset (int grp);
extern void		set_weland_offset (int grp, void* data);
extern unsigned int	get_fizone_height (int grp);
extern void		set_fizone_height (int grp, void* data);
extern unsigned int	get_fizone_angle (int grp);
extern void		set_fizone_angle (int grp, void* data);
extern unsigned int	get_fizone_radius (int grp);
extern void		set_fizone_radius (int grp, void* data);
...
extern unsigned int get_weld_display() ;
extern void         set_weld_display(unsigned int data);
/* 自动检测探头 */
...
extern unsigned int	get_file_save_mode (CONFIG *p);
...
extern char	*get_file_name_all (CONFIG *p);
 ...
 ***/
extern void RequestProbeAutoDetect();
extern char get_file_type (CONFIG *p);
extern void set_file_type (CONFIG *p, char data);
